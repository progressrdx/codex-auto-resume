...
> [!IMPORTANT]
> 这是非官方实验工具，与 OpenAI 没有关联或背书。“真实额度耗尽 → 跨自然重置窗口 → 自动确认续跑”的完整周期尚未完成端到端验证，详见[当前验证状态](#当前验证状态)。
...
在同一台 Mac 的浏览器打开地址并输入凭据。服务固定监听 `127.0.0.1`，不会暴露到局域网或公网。
...
- 真实账户从自然额度耗尽、跨过真实重置窗口，到自动续跑得到确认的完整周期；
