...
- To isolate query connectivity on an unverified App version, run `./resume probe` (Windows: `.\resume.cmd probe`). It only requests account rate limits, does not read tasks, and does not authorize resuming. `resumeVerified` remains false; never replace doctor/check with probe for enrollment.
