        ...
        try:
            with self.desktop_factory() as desktop:
                ...
                if decision.action == 'wait':
                    self.failures = 0
                    self.store.update(self.thread_id, 'watching' if decision.task_state == 'running' else 'waiting_connection', decision.reason)
                    return True
                ...
                if previous:
                    if previous['state'] == 'sent' and self.clock() - previous['created'] < 120:
                        self.store.update(self.thread_id, 'watching', '续跑已发送，等待 App 更新状态')
                        return True
                ...
                if self.clock() < self.next_quota_check:
                    self.store.update(self.thread_id, 'waiting_quota', '等待额度重置；期间仍检查任务是否被停止')
                    return True
                stage = 'quota'
                ...
                if not ready:
                    ...
                    self.store.update(self.thread_id, 'waiting_quota', reason)
                    return True
                ...
                self.waiting_baseline = None
                self.next_quota_check = 0
                self.store.update(self.thread_id, 'resumed', 'App 已确认启动下一轮；继续观察结果')
                return True
        except (ConnectionUnavailable, ConnectionError, FileNotFoundError, TimeoutError) as exc:
            ...
            self.store.update(self.thread_id, status, reason)
            return True
        except Exception:
            self.failures += 1
            if self.failures >= 3:
                return self.end('blocked', '连续三次无法安全读取 App 或额度；请检查 App、登录、网络与版本')
            self.store.update(self.thread_id, 'retrying', '暂时无法读取状态；不续跑，稍后重查')
            return True

    def end(self, status, reason):
        self.store.update(self.thread_id, status, reason, True)
        return False
